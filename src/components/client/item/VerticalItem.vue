<template>
    <div class="flex flex-col overflow-hidden rounded-lg shadow-lg" >
        <div class="flex-shrink-0">
            <img class="h-48 w-full object-cover" :src="this.book.image.path" alt="" />
        </div>
        <div class="flex flex-1 flex-col justify-between bg-white dark:bg-zinc-700 p-6 relative group" >
            <button
                class="md:hidden absolute md:top-2 right-2 group-hover:inline p-1 bg-gray-200 dark:bg-white rounded-full hover:rotate-12">
            ...
            <div class="flex-1">
                <router-link :to="{name: 'BookDetail', params: { slug: this.book.slug }}" class="mt-2 block">
                ...
                <div class="flex gap-1 flex-wrap">
                    <router-link v-for="category in this.book.categories" :key="category.id" :to="{name: 'BookListCategory', params: { slug: category.slug }}"
            ...
            <div class="flex flex-col md:flex-row justify-between md:items-center mt-6 gap-y-4">
                <div class="text-sm font-normal">
                    {{ this.book.count }} <span>đã mượn</span>
                </div>
                <div class="flex space-x-1 text-sm text-gray-500 dark:text-white">
                    <span :comments="this.book.count" class="flex items-center gap-x-1">{{ this.book.count }}
                    ...
                    <span class="flex items-center gap-x-1">{{ this.book.quantity }}
