<template>
    <div class="mx-auto px-4 sm:px-6 md:px-8 dark:bg-zinc-700 rounded-lg dark:text-white bg-white text-black">
        <div class="py-4 sm:py-6 md:py-8 flex flex-col">
            <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                    <div class="overflow-hidden">
                        <MenuComponent resource="tag" title="Nhãn Dán Hiện Trên Menu"/>
                        <MenuComponent resource="category" title="Thể Loại Hiện Trên Menu"/>
                        <MenuComponent resource="author" title="Tác Giả Hiện Trên Menu"/>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import MenuComponent from '@/components/admin/MenuComponent.vue';

    export default {
    components: { MenuComponent }
}
</script>
